...
def _grid_columns(action: Mapping[str, Any]) -> int:
    ...
    raise RenderError(f"actions_home_v1 unsupported action kind {kind!r}")


def _role_entity(semantic_module: Mapping[str, Any], role_name: str) -> str:
    roles = semantic_module.get("roles")
    if not isinstance(roles, list):
        raise RenderError("actions_home_v1 semantic roles missing")
    for role in roles:
        ...
        if isinstance(entity_id, str) and entity_id:
            return entity_id
    raise RenderError(f"actions_home_v1 role {role_name!r} missing")
...
def _vacuum_command(
    ...
    *,
    ...
    if service not in {"vacuum.start", "vacuum.return_to_base"}:
        raise RenderError(f"actions_home_v1 unsafe vacuum service {service!r}")
    return {
        "type": "custom:mushroom-template-card",
        "entity": entity_id,
        "primary": primary,
...
def render_actions_dashboard(
    ...
    if not isinstance(views, list) or not isinstance(semantic_views, list):
        raise RenderError("actions_home_v1 requires rendered views and semantic trace views")
    if len(views) != len(semantic_views):
        raise RenderError("actions_home_v1 view/trace count mismatch")

    for view, semantic_view in zip(views, semantic_views, strict=True):
        if not isinstance(view, dict) or view.get("type") != "masonry":
            raise RenderError("actions_home_v1 expects validated tiles_v1 masonry input")
        cards = view.pop("cards", None)
        modules = semantic_view.get("modules")
        if not isinstance(cards, list) or not isinstance(modules, list):
            raise RenderError("actions_home_v1 module input missing")
        if len(cards) != len(modules) * 2:
            raise RenderError("actions_home_v1 heading/grid pairs do not match modules")
        ...
        for module_index, semantic_module in enumerate(modules):
            ...
            if not isinstance(heading, dict) or heading.get("type") != "heading":
                raise RenderError("actions_home_v1 module must begin with a heading")
            if not isinstance(grid, dict) or grid.get("type") != "grid":
                raise RenderError("actions_home_v1 module must contain a tile grid")
            if not isinstance(semantic_module, dict):
                raise RenderError("actions_home_v1 invalid semantic module")

            tiles = grid.get("cards")
            roles = semantic_module.get("roles")
            if not isinstance(tiles, list) or not isinstance(roles, list):
                raise RenderError("actions_home_v1 roles/tiles missing")
            if len(tiles) != len(roles):
                raise RenderError("actions_home_v1 role/tile count mismatch")
            ...
            for tile, role in zip(tiles, roles, strict=True):
                if not isinstance(tile, dict) or not isinstance(role, dict):
                    raise RenderError("actions_home_v1 invalid role/tile")
                action = role.get("action")
                if not isinstance(action, dict):
                    raise RenderError("actions_home_v1 role action missing")
            ...
            if len(section_cards) <= 1:
                raise RenderError("actions_home_v1 module rendered no actions")
        ...
        if not sections:
            raise RenderError("actions_home_v1 rendered no sections")
        view["type"] = "sections"
