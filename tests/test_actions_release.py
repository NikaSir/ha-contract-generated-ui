...
def test_release_version_and_schema_are_packaged() -> None:
    ...
    assert manifest["version"] == "0.36.6"
...
def test_frontend_bundle_and_generated_panel_hosts_are_packaged() -> None:
    ...
    assert 'const BOOTSTRAP_VERSION = "b012"' in bundle
    ...
    assert 'import "/contract_generated_ui/frontend/nikas-house-hero.js?build=b012"' in bundle
    ...
    assert 'UI_BUNDLE_BUILD = "b012"' in const_source
    ...
    assert 'HOUSE_HERO_BUILD = "b012"' in const_source
    assert 'HOUSE_PANEL_FILENAME = "dist/nikas-house-overview.js"' in const_source
    assert 'HOUSE_PANEL_BUILD = "b009"' in const_source
