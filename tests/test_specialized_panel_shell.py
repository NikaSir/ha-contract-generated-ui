...
def test_specialized_shell_contract_is_documented() -> None:
    ...
    assert "Specialized Panel Shell Standard v1.4" in shell_doc
    ...
    assert "exactly one active canvas viewport" in zoom_doc
    assert "translate3d(x,y,0) scale(s)" in zoom_doc
    assert "native overflow scrolling" in zoom_doc
    assert "pan_at_100_percent: true" in zoom_doc
    assert "restore_before_paint: true" in zoom_doc
    assert "pointercancel" in zoom_doc
    ...
    assert "Stark SolarPower 1.8.10" in lessons_doc
    ...
    assert "CSS `zoom`" in lessons_doc
    assert "pointercancel" in lessons_doc
