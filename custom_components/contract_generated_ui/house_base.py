    ...
    cards.append({
        ...
        "filter": {
            "template": (
                ...
                "{% set pe=namespace(v=[],bad=0) %}{% for e in " + _literal(power) + " %}{% if is_number(states(e)) %}{% set pe.v=pe.v+[states(e)|float] %}{% else %}{% set pe.bad=pe.bad+1 %}{% endif %}{% endfor %}{% if pe.bad==0 and pe.v|count==3 %}{% if (pe.v|min)<198 or (pe.v|max)>242 %}{% set ns.cards=ns.cards+[{'type':'custom:mushroom-template-card','primary':'Электросеть дома — авария','secondary':'Проверить напряжение','icon':'mdi:home-lightning-bolt','icon_color':'red','tap_action':{'action':'navigate','navigation_path':'" + _nav(manifest, "electricity") + "'},'grid_options':{'columns':12}}] %}{% elif (pe.v|min)<205 or (pe.v|max)>235 %}{% set ns.cards=ns.cards+[{'type':'custom:mushroom-template-card','primary':'Электросеть дома — отклонение','secondary':'Проверить напряжение','icon':'mdi:home-lightning-bolt-outline','icon_color':'orange','tap_action':{'action':'navigate','navigation_path':'" + _nav(manifest, "electricity") + "'},'grid_options':{'columns':12}}] %}{% endif %}{% endif %}"
