...
`house_home_v1` is the first staged renderer for migrating the Home Assistant NikaS `Дом` start page into Contract Generated UI without replacing the current production dashboard before visual acceptance.
...
It does **not** automatically install a House contract, House panel manifest or private House SemanticInventory into `/config/contract_generated_ui`, and therefore does not replace `/dashboard-house`.
...
1. Update Contract Generated UI to `0.8.0` and restart Home Assistant.
...
4. Copy the activation sources into `/config/contract_generated_ui/` without modifying the existing infrastructure sources or private inventory files.
